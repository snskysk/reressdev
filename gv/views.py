...
def mainhome(request):
    if request.method == 'POST':
        ...
        #正しくなかったら戻る
            ...
            return render(request, 'gv/index.html', params1)

        #正しくデータを整形することができるかどうか。
        try:
            #pythonからjsへの値の受け渡し
            global params2
            params2 = pytojsMaterials(result, list_pie, list_bar, table)

            #ほかの関数でも使えるかどうか
            global str_name
            str_name = params2['str_name']
            global gradeAchievement
            gradeAchievement = params2['gradeAchievement']
        ...
        #後々新しいpyファイルを作成する
        #gpaの順位
        gpaobj = studentInfo.objects.filter(gpa__gt = float(params2['gpa']))
        rank = len(gpaobj) + 1
        params2['rank'] = rank
        return render(request, 'gv/mainhome.html', params2)
        #return render(request, 'gv/main.html', params)
    else:
        form = userInfoForm()
        params1 = {
        'form':form,
